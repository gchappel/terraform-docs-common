...
[terraform_cloud_for_business]: https://cloud.hashicorp.com/docs/hcp/admin/billing/flex-billing#terraform-cloud-for-business
...
You can disable remote operations for any workspace by changing its [Execution Mode][execution_mode] to **Local**. This causes the workspace to act only as a remote backend for Terraform state, with all execution occurring on your own workstations or continuous integration workers. Consequently, any [Apply][terraform_cloud_for_business] that is run in Local Execution mode will not count towards any Flex meters or Service Entitlements.
