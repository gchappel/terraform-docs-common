...
Run tasks perform actions between the [plan](/cloud-docs/run/states#4-the-plan-stage) and [apply](/cloud-docs/run/states#9-the-apply-stage) stages of a [Terraform run](/cloud-docs/run). Once all run tasks have completed, the run will end based on the most restrictive enforcement level in each associated run task.
