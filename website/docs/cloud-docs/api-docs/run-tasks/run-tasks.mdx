...
You may also configure the run task to begin during a specific [run stage](/cloud-docs/run/states). Run tasks use the [Post-Plan Stage](/cloud-docs/run/states#5-the-post-plan-stage) by default.
...
| `data.attributes.stage`             | string | `"post_plan"` | The stage in the run lifecycle when the run task should begin. Must be `"pre_plan"`, `"post_plan"`, `"pre_apply"`. |
